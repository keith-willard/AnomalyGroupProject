...
def run_devnet(args):
    ...
    for nm in names:
        ...
        for i in np.arange(runs):
            ...
            datum = np.append(scores, y_test, axis=1)

            # print(datum.shape, datum)
            # exit(1)

            df = pd.DataFrame(data=datum, columns=['score', 'type'])
            df.loc[df['type'] == 0.0, 'type'] = "normal"
            df.loc[df['type'] == 1.0, 'type'] = "anomaly"
            ...
            print("Plotting box")
            sns.boxplot(x="score", y="type", data=df, width=.6, fliersize=0., palette="vlag")
            print("Plotting score")
            sns.stripplot(x="score", y="type", data=df, size=4, color=".3", linewidth=1, alpha=0.2)
